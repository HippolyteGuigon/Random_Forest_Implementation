...
from Random_forest.criterion.criterion import (
    ...
    compute_gini_numerical,
    variance_reduction_numerical,
    variance_reduction_categorical,
)
from Random_forest.decision_tree.array_functions import (
    treshold_numeric,
    split_categorical,
    is_float,
)
...
class Node:
    ...
    def compute_condition(self) -> None:
        ...
        variance_reduction = []
        gini_scores = []
        chosen_column=np.random.choice(np.arange(0, self.X.shape[1]),
                                       replace=False, size=(int(self.nb_features),1)).flatten()
        ...
        for col in chosen_column:
            if is_float(self.X[0, col]):
                ...
                if isinstance(self.y.flatten()[0], (np.int_, np.float_)):
                    variance = variance_reduction_categorical(
                        self.X[:, col].reshape(-1, 1), self.y
                    )
                    variance_reduction.append(variance)
                else:
                    gini = gini_impurity_categorical(
                        self.X[:, col].reshape(-1, 1), self.y
                    )
                    gini_scores.append(gini)
        ...
        for score_couple_index in range(len(criterion_scores)):
            if isinstance(criterion_scores[score_couple_index][0], str):
                (
                    criterion_scores[score_couple_index][0],
                    criterion_scores[score_couple_index][1],
                ) = (
                    criterion_scores[score_couple_index][1],
                    criterion_scores[score_couple_index][0],
                )
    ...
    def get_data_subsets(self) -> None:
        ...
        if not hasattr(self, "split_value"):
            raise NotFittedError(
                "The condition for this Node needs to be computed first"
            )
        ...
        if self.condition == treshold_numeric:
            X_left_node = self.X[
                vf(
                    self.X[:, self.split_column].astype(float),
                    reference_value=self.split_value,
                )
            ]
            X_right_node = self.X[
                ~(
            ...
            X_left_node = self.X[
                vf(self.X[:, self.split_column], reference_value=self.split_value)
            ]
            X_right_node = self.X[
                ~(vf(self.X[:, self.split_column], reference_value=self.split_value))
            ]
...
class Decision_Tree:
    ...
    def grow_node(self, node) -> None:
        ...
        if (node.X.shape[0] >= self.min_samples_split) and (
            node.profondeur < self.max_depth
        ):
