...
def gini_impurity_categorical(X: np.array, y: np.array) -> tuple:
    ...
    for value in unique_value:
        ...
        for unique_subtarget in unique_target:
            gini_impurity -= (
                X_filtered[X_filtered == unique_subtarget].shape[0] / n_value
            ) ** 2
...
def compute_gini_numerical(X: np.array, y: np.array) -> tuple:
    ...
    tresholds = np.array(
        [
            (np.float(full_data[i, 0]) + np.float(full_data[i + 1, 0])) / 2
    ...
    for treshold in tresholds:
        ...
        for value_left, value_right in zip(unique_target_left, unique_target_right):
            impurity_right -= (
                right_node[right_node == value_right].shape[0] / n_right_node
            ) ** 2
            impurity_left -= (
                left_node[left_node == value_left].shape[0] / n_left_node
            ) ** 2
        total_impurity = impurity_right * (n_right_node / n) + impurity_left * (
            n_left_node / n
        )
...
def variance_reduction_numerical(X: np.array, y: np.array) -> Tuple[float, float]:
    ...
    for treshold_candidate in tresholds:
        left_node = full_data[full_data[:, 0].astype(float) < treshold_candidate][
            :, 1
        ].astype(float)
        right_node = full_data[full_data[:, 0].astype(float) >= treshold_candidate][
            :, 1
        ].astype(float)
        n_left_node = len(left_node)
        n_right_node = len(right_node)
        total_variance = (n_left_node / n) * np.var(left_node) + (
            n_right_node / n
        ) * np.var(right_node)
...
def variance_reduction_categorical(X: np.array, y: np.array) -> tuple:
    ...
    for candidate in split_candidate:
        ...
        total_variance = (n_first_subset / n) * np.var(first_subset) + (
            n_second_subset / n
        ) * np.var(second_subset)
...
def full_gini_compute(X: np.array, y: np.array) -> tuple:
    ...
    full_gini_impurities = [
        (col, gini_impurity_categorical(X[:, col].reshape(-1, 1), y))
        for col in range(X.shape[1])
    ]
    min_index = np.argmin([x[1][0] for x in full_gini_impurities])
...
def full_variance_reduction_compute(X: np.array, y: np.array) -> tuple:
    ...
    best_candidates = [
        (col, variance_reduction_categorical(X[:, col].reshape(-1, 1), y))
        for col in range(X.shape[1])
    ]
